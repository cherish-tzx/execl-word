...
export default {
  ...
  methods: {
    ...
    getColumnMapping(leftRow, rightRow) {
      ...
      // 按位置匹配未匹配的列
      const minUnmatched = Math.min(
        unmatchedLeft.length,
        unmatchedRight.length
      );
      for (let k = 0; k < minUnmatched; k++) {
        const leftIdx = unmatchedLeft[k];
        const rightIdx = unmatchedRight[k];
        leftToRight[leftIdx] = rightIdx;
