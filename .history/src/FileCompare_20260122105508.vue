        ...
        for (let r = 0; r < maxRows; r++) {
          ...
          for (let c = 0; c < maxCols; c++) {
            ...
            if (diff.type === "equal") {
              ...
              // 左侧有，右侧无 - 标记为删除（红色）
              rightRowHtml += `<td class="deleted-cell">${this.escapeHtml(
                rightDisplay
              )}</td>`;
            } else if (diff.type === "add") {
              // 左侧无，右侧有 - 标记为新增（绿色）
              rightRowHtml += `<td class="added-cell">${this.escapeHtml(
                rightDisplay
              )}</td>`;
            } else {
              // 内容不同 - 标记为修改（橙色）
              rightRowHtml += `<td class="modified-cell">${this.escapeHtml(
                rightDisplay
              )}</td>`;
            }
          }
...
/* 右侧表格的颜色标记 - 使用更强的优先级 */
.excel-table td.deleted-cell {
  background-color: #ffcdd2 !important;
}

.excel-table td.added-cell {
  background-color: #c8e6c9 !important;
}

.excel-table td.modified-cell {
