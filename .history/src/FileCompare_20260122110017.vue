      ...
      for (let i = 0; i < maxSheets; i++) {
        ...
        for (let r = 0; r < maxRows; r++) {
          ...
          for (let c = 0; c < maxCols; c++) {
            ...
            const diff = this.compareCellValues(leftValue, rightValue);

            // 左侧：始终显示原始内容，不带颜色标记
            const leftDisplay = leftCell.value || "";
            leftRowHtml += `<td>${this.escapeHtml(leftDisplay)}</td>`;

            // 右侧：显示原始内容，但根据差异类型添加颜色标记
            const rightDisplay = rightCell.value || "";

            if (diff.type === "equal") {
              matchedCells++;
              rightRowHtml += `<td>${this.escapeHtml(rightDisplay)}</td>`;
            } else if (diff.type === "delete") {
              // 左侧有，右侧无 - 标记为删除（红色）
              rightRowHtml += `<td class="deleted-cell" style="background-color: #ffcdd2 !important;">${this.escapeHtml(
                rightDisplay
              )}</td>`;
            } else if (diff.type === "add") {
              // 左侧无，右侧有 - 标记为新增（绿色）
              rightRowHtml += `<td class="added-cell" style="background-color: #c8e6c9 !important;">${this.escapeHtml(
                rightDisplay
              )}</td>`;
            } else {
              // 内容不同 - 标记为修改（橙色）
              rightRowHtml += `<td class="modified-cell" style="background-color: #ffe0b2 !important;">${this.escapeHtml(
                rightDisplay
              )}</td>`;
            }
          }
